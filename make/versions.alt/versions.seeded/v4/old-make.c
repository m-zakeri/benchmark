...
#define ALIASPATH "/usr/local/share/locale:."  /* modification by J. Ruthruff, 8/25 */
#define LOCALEDIR "/usr/local/share/locale"  /* modification by J. Ruthruff, 8/25 */
#define LIBDIR "/usr/local/lib"  /* modification by J. Ruthruff, 8/25 */
#define INCLUDEDIR "/usr/local/include"  /* modification by J. Ruthruff, 8/25 */
#define HAVE_CONFIG_H  /* modification by J. Ruthruff, 8/25 */
...
#undef stderr
#define stderr stdout
size_t int_to_size_t(long int x)
{
  size_t y;
  memcpy(&y, &x, sizeof(size_t));
  return y;
}

off_t int_to_off_t(long int x)
{
  off_t y;
     ...
     long int arg;
{
...
#ifdef AIAMAG
  {
	...
	if (off_t_to_int(lseek (desc, int_to_off_t(0), 0)) < 0)
	  /*	if (lseek (desc, 0, 0) < 0) */
	  {
	    (void) close (desc);
	    return -2;
  ...
  /* Now find the members one by one.  */
  {
	...
	if (off_t_to_int(lseek (desc, int_to_off_t(member_offset), 0)) < 0)
     ...
     char *arname, *memname;
{
  ...
  if (off_t_to_int(lseek (fd, int_to_off_t(pos), 0)) < 0)
    /*  if (lseek (fd, pos, 0) < 0) */
    goto lose;
...
{
  register char *p;
#ifndef F_JR_6  /* seeded faults #6; J.Ruthruff */
...
#ifndef F_JR_7  /* seeded faults #7; J.Ruthruff */
...
# else
	  hash = ((unsigned int) (dev_t_to_int(st.st_dev)) << 16) | (unsigned int) (ino_t_to_int(st.st_ino));
...
# else
                && ino_t_to_int(dc->ino) == ino_t_to_int(st.st_ino)
		/*                && dc->ino == st.st_ino */
# endif
                )
#endif /* WINDOWS32 */
  ...
  for (i = 0; i < DIRECTORY_BUCKETS; ++i)
	...
	if (dir->contents == 0)
	  ...
	  printf (_("# %s (device %ld, inode %ld): could not be opened.\n"),
		  ...
		  (long int) (ino_t_to_int(dir->contents->ino)));
	...
	else
	  {
	    ...
	    printf (_("# %s (device %ld, inode %ld): "),
		    dir->name,
                    (long)(dev_t_to_int(dir->contents->dev)), (long)(ino_t_to_int(dir->contents->ino)));
     ...
     register struct variable *v;
{
  ...
  v->expanding = 1;
#ifndef F_JR_10  /* seeded fault #10 */
  if (v->append)
#else
  if (!(v->append))
#endif
     ...
     struct variable *v;
{
  ...
  variable_buffer = 0;
#else
  variable_buffer = (char *) 10;  /* was variable_buffer = 10; had to cast 
                                     to get CFE to compile; J.Ruthruff */
#endif
...
#ifndef F_JR_8  /* seeded fault #8; J.Ruthruff */
  child->file->cmds->lines_flags[child->command_line - 1] |= flags;
#else
  child->file->cmds->lines_flags[child->command_line] |= flags;
#endif
...
#ifndef F_JR_9  /* seeded fault #9; J.Ruthruff */
      ...
      while (1)
        {
          ...
          while (*(++p) != '\0')
            if (*p == ',' || *p == ' ')
              break;
#ifndef F_JR_1  /* seeded fault #1; J.Ruthruff */
          if (*p == '\0')
#else
          if (*p != '\0')
#endif 
  ...
  if (read_makefiles != 0)
    {
      ...
      char **nargv = argv;
      int nargc = argc;
#ifndef F_JR_3  /* seeded fault #3; J.Ruthruff */
     ...
     unsigned int len;
{
  ...
  while (*value != '\0')
    {
#ifndef F_JR_4  /* seeded fault #4; J.Ruthruff */
      if (*value == '\\' && value[1] != '\0')
#else
      if (*value == '\\')
#endif
  ...
  while (*in != '\0')
    {
      if (*in == '$')
	*out++ = '$';
#ifndef F_JR_5  /* seeded fault #5; J.Ruthruff */
...
#else
  for (i = 0; default_include_directories[i+1] != 0; ++i)
#endif
     ...
     register struct file *file;
{
  ...
  if (ar_name (file->name))
      ...
      else
	{
	  ...
	  if (off_t_to_int(lseek (fd, int_to_off_t(0L), 0)) < 0L)
	    /*	  if (lseek (fd, 0L, 0) < 0L) */
	  ...
	  if (off_t_to_int(statbuf.st_size) == 0)
     ...
     struct loaded_l10nfile *domain_file;
{
  ...
  if (fstat (fd, &st) != 0
      || (size = int_to_size_t(off_t_to_int(st.st_size))) != off_t_to_int(st.st_size)
