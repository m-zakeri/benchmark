...
#define LOCALEDIR "/nfs/spectre/u4/jin/research/experiments/grep/docs/grep_2.4.1/lib/locale"
...
grep (int fd, char const *file, struct stats *stats)
{
...
#else
  not_text = ((binary_files == WITHOUT_MATCH_BINARY_FILES || !out_quiet)
#endif
  ...
  if (not_text && binary_files == WITHOUT_MATCH_BINARY_FILES)
    return 0;
  #else
	/* Nothing here because the fault is only a code removal */
  #endif
	  ...
	  do
	      ...
	      for (c = 0; c < NOTCHAR; c++)
		{
		  ...
		  if (strcoll (lo, ch) <= 0 && strcoll (ch, hi) <= 0)
                  #else
		  if (strcoll (ch, lo) <= 0 && strcoll (hi, ch) <= 0)
                  #endif
		    {
		      setbit (c, ccl);
		      if (case_fold)
