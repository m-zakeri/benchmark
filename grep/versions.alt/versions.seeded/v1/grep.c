...
/**** I added all things in Makefile in order to run cfe *******/
#define HAVE_CONFIG_H 1
#define LOCALEDIR "/nfs/spectre/u4/jin/research/experiments/grep/docs/grep_2.2/lib/locale"
#define _LARGEFILE_SOURCE 1
#define _FILE_OFFSET_BITS 32 /* In order to let cfe work, change original 64 to 32*/
                             /* Reason: Please see /usr/include/sys/types.h */
/*typedef int off_t; */     /** since cfe can't recognize off_t defined in system file as int*/
			/* so I change all off_t to Off_t and define type of Off_t */ 
/****** end for cfe ***************/
...
#else
    cc = undossify_input(bufalloc - bufsalloc, cc);
#endif
#endif
  if (cc > 0)
  ...
  for (;;)
      ...
      if (beg < lim)
	{
	  ...
	  if (nlines && done_on_match && !out_invert)
	    return nlines;
          #else
	  if (nlines && done_on_match && !out_quiet)
	    return nlines;
          #endif
	}
      i = 0;
		...
		for (c1 = 0; prednames[c1].name; ++c1)
		  if (looking_at(prednames[c1].name))
			...
			if (case_fold
				...
				pred = is_alpha;
			#else
			if (pred == is_upper || pred == is_lower)
				pred = is_alpha;
			#endif
...
#else
  const char *next_next = p + 1 < pend ? 0 : p + 1;
#endif
