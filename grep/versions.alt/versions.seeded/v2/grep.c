...
/**** In order to run cfe, I added follow things from makefile *****/ 
#define HAVE_CONFIG_H 1
#define LOCALEDIR "/nfs/spectre/u4/jin/research/experiments/grep/docs/grep_2.3/lib/locale"
#define _LARGEFILE_SOURCE  1
#define _FILE_OFFSET_BITS 32   /** change from 64 to 32 ***/
/**** end of adding for cfe *********/
     ...
     struct stats *stats;
{
  ...
  if (!S_ISREG (stats->stat.st_mode))
  ...
  else
    {
	  ...
	  #else
      bufoffset = initial_bufoffset = lseek (fd, 0, 1);
	  #endif
    }
#endif
  return 1;
     ...
     struct stats *stats;
{
  ...
  if (save > bufsalloc)
	...
	fatal (_("memory exhausted"), 0);
      ...
      bufbeg = nbuffer + bufsalloc - save;
      memcpy (buflim - save, bufbeg, save);
      free (ubuffer);
      ubuffer = nubuffer;
      buffer = nbuffer;
      #endif
     ...
     struct stats *stats;
{
 ...
 finish_grep:
  ...
  out_quiet -= not_text;
 #else
	/* nothing here because the fault is just code removal */
 #endif
